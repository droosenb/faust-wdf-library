...
function mathmatica2faustArray(str::String, f::String, dim::Int)
    #return a faust scattering matrix from a mathmatica matrix (RawInputForm)
    str = replace(str, "{" => "")
    str = replace(str, "}" => "")
    tmp = tmp = "\n\n$f  = \ncase{\n\t(1, 1) => "
    entry = 1;
...
#tonestack MNA varibles
@variables Ra Rb Rc Rd Re Rf Va Vb Vc Vd Ve Vf rho;

#initialize array of zeros before performing MNA
MNA = zeros(Num, 16, 16)

#declare node connections of the resistors and voltage sources
R_mtx = [Ra 1 7
         Rb 2 7
         Rc 3 9
         Rd 4 10
...
# remove ground node
gnd = 10
MNA = MNA[1:end .!= gnd, 1:end .!= gnd]

#print the MNA array as a mathmatica form
println(julia2mathmaticaArray(MNA))

#generate arrays of zeros and 1's which will reduce the mattrix
Red =  Array([zeros(Num, 9, 6)
        Diagonal(ones(Num, 6))])

#print reducing array in mathmatica form
println(mathmaticaArray(Red))

#perform remaining computations in Mathmatica and copy back in matrix in RawInputForm
...
#print the array as a faust scattering matrix function
